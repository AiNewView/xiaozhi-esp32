...
#define EMT_DEF_ELEM_MOUTH_ANIM        "mouth_anim"
#define EMT_DEF_ELEM_MOUTH_ICON        "mouth_icon"
...
/**
 * @brief Set mouth emoji animation by emotion name
 *        Maps emotion to appropriate mouth EAF (happy→Happy.eaf, sad→Sad.eaf, etc.)
 * @param handle Handle to emote manager
 * @param emotion Emotion name (e.g., "happy", "sad", "angry", "neutral")
 * @return ESP_OK on success, error code on failure
 */
esp_err_t emote_set_mouth_emotion(emote_handle_t handle, const char *emotion);

/**
 * @brief Mouth animation tick — toggles between emotion mouth and talking mouth
 *        Call periodically (e.g., from status timer) only when mouth is visible.
 * @param handle Handle to emote manager
 */
