...
// Mouth helper
static const char *emote_get_mouth_emoji(const char *emotion);
...
// Mouth emoji mapping: emotion name → emoji EAF name
static const char *emote_get_mouth_emoji(const char *emotion)
{
    if (!emotion) {
        return "neutral";
    ...
    typedef struct { const char *emotion; const char *mouth; } mouth_map_t;
    static const mouth_map_t mouth_map[] = {
        {"happy",       "happy"},
        {"laughing",    "happy"},
        {"funny",       "happy"},
        {"loving",      "happy"},
        ...
        {"neutral",     "neutral"},
        {"idle",        "neutral"},
        {"microchip_ai","winking"},
    };

    for (size_t i = 0; i < sizeof(mouth_map) / sizeof(mouth_map[0]); i++) {
        if (strcmp(emotion, mouth_map[i].emotion) == 0) {
            return mouth_map[i].mouth;
        }
    }
    return "neutral";
}
...
static esp_err_t emote_handle_listen_event(emote_handle_t handle, const char *message)
{
    ...
    ret = emote_set_icon_image(handle, EMOTE_DEF_OBJ_ICON_MOUTH, handle->mouth_emoji, true);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Failed to set mouth icon: %s", handle->mouth_emoji);
    }
    handle->mouth_animating = true;
    handle->mouth_toggle = false;
...
esp_err_t emote_set_mouth_emotion(emote_handle_t handle, const char *emotion)
{
    if (!handle || !emotion) {
        return ESP_ERR_INVALID_ARG;
    }

    const char *mouth_name = emote_get_mouth_emoji(emotion);
    // Prefix with "mouth_" to match icon names (mouth_happy, mouth_sad, etc.)
    snprintf(handle->mouth_emoji, sizeof(handle->mouth_emoji), "mouth_%s", mouth_name);

    ESP_LOGI(TAG, "Set mouth emotion: %s -> %s", emotion, handle->mouth_emoji);

    // Immediately update the mouth icon (visible=false so it only shows during speak/listen)
    emote_set_icon_image(handle, EMOTE_DEF_OBJ_ICON_MOUTH, handle->mouth_emoji, false);

    return ESP_OK;
}

void emote_animate_mouth_tick(emote_handle_t handle)
{
    if (!handle || !handle->mouth_animating) {
        return;
    }

    handle->mouth_toggle = !handle->mouth_toggle;
    const char *mouth_name = handle->mouth_toggle ? "mouth_talking" : handle->mouth_emoji;
    emote_set_icon_image(handle, EMOTE_DEF_OBJ_ICON_MOUTH, mouth_name, true);
}
...
esp_err_t emote_insert_anim_dialog(emote_handle_t handle, const char *name, uint32_t duration_ms)
{
    ...
    timer = gfx_timer_create(handle->gfx_emote_handle, emote_dialog_timer_cb, duration_ms, handle);
...
esp_err_t emote_set_event_msg(emote_handle_t handle, const char *event, const char *message)
{
    ...
    if (!entry->skip_hide_ui) {
        ...
        HIDE_OBJ(handle, EMOTE_DEF_OBJ_ICON_MOUTH);
...
esp_err_t emote_notify_flush_finished(emote_handle_t handle)
{
    ...
    ESP_GOTO_ON_FALSE(handle->gfx_emote_handle, ESP_ERR_INVALID_STATE, error, TAG, "GFX handle not initialized");
