...
emote_handle_t emote_init(const emote_config_t *config)
{
    ...
    strncpy(handle->mouth_emoji, "mouth_neutral", sizeof(handle->mouth_emoji) - 1);
...
bool emote_deinit(emote_handle_t handle)
{
    ...
    if (handle->gfx_emote_handle) {
        ...
        for (int i = 0; i < EMOTE_DEF_OBJ_MAX; i++) {
            ...
            } else if (i == EMOTE_DEF_OBJ_ICON_MOUTH || i == EMOTE_DEF_OBJ_ICON_STATUS || i == EMOTE_DEF_OBJ_ICON_CHARGE) {
                if (entry->data.img) {
